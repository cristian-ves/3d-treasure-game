...
struct NodeOL
{
    ...
    NodeOL(const T &value) : data(value), up(nullptr), down(nullptr), left(nullptr), right(nullptr), front(nullptr), back(nullptr) {}
};
...
public:
    OrthogonalList(int rows, int cols, int layers) : head(nullptr), rows(rows), cols(cols), layers(layers)
    {
        initializeEmptyList();
    }
    ...
    NodeOL<T> *getHead() const { return head; }
};
